...
writing; `pnpm run lint` applies Biome's recommended lint rules to it and fails
on warnings as well as errors. CI checks the entire adopted list on Linux and
...
formats and lints staged files from the same list and leaves every other file
untouched.
