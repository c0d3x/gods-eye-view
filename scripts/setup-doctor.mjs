...
const LAUNCHER_SOURCES = new Set(['macOS Keychain', 'OpenSky credentials file']);
...
export function checkPnpmVersion(version, packageManager) {
  const pinned = /^pnpm@(\d+\.\d+\.\d+)/.exec(String(packageManager || ''))?.[1];
  if (!pinned) return { level: 'ok', summary: 'package.json pins no version' };
  if (version === pinned) return { level: 'ok', summary: 'the version package.json pins' };
  return {
    level: 'warn',
...
export function checkLockfile(rootDir = ROOT) {
  ...
  if (installed === null) {
    return { level: 'warn', summary: 'node_modules holds no pnpm install', hint: 'Run pnpm install.' };
  }
  if (lockfileLines(installed).join('\n') !== lockfileLines(lockfile).join('\n')) {
    return {
      level: 'warn',
...
export function checkWs(rootDir = ROOT) {
  ...
  return { level: 'ok', summary: `${version} loads for the AISStream vessel relay` };
}
  ...
  return new Promise((resolve) => {
    ...
    server.once('error', (error) => {
      if (error.code === 'EADDRINUSE') {
        ...
        resolve({ level: 'warn', summary: `not allowed on ${host}`, hint: 'Set PORT to a port above 1023.' });
      } else if (['EADDRNOTAVAIL', 'ENOTFOUND', 'EAI_AGAIN'].includes(error.code)) {
        resolve({
          level: 'warn',
        ...
        resolve({ level: 'warn', summary: `could not be checked on ${host} (${error.code || error.message})` });
      }
    });
    try {
      server.listen({ host, port }, () => {
        server.close(() => resolve({ level: 'ok', summary: `free on ${host}` }));
      });
    } catch {
      resolve({ level: 'warn', summary: 'is not a valid port', hint: 'Set PORT to a number from 1 to 65535.' });
    }
  });
}
 ...
 */
export function checkGitHook({ rootDir = ROOT, environment = process.env, spawn = spawnSync } = {}) {
  const ci = String(environment.CI || '').toLowerCase();
  if (ci && ci !== 'false' && ci !== '0') return { level: 'info', summary: 'not checked in CI' };
  const result = spawn('git', ['rev-parse', '--git-path', 'hooks'], { cwd: rootDir, encoding: 'utf8' });
  if (result.status !== 0) return { level: 'info', summary: 'no Git checkout found' };
  let hook = '';
  try {
    hook = readFileSync(path.join(path.resolve(rootDir, String(result.stdout).trim()), 'pre-commit'), 'utf8');
  } catch {
    // No pre-commit hook.
  }
  if (/lefthook/i.test(hook)) return { level: 'ok', summary: "Lefthook's pre-commit hook is installed" };
  return {
    level: 'warn',
    summary: hook ? "the pre-commit hook is not Lefthook's" : "Lefthook's pre-commit hook is not installed",
...
export async function checkQaBrowser({
  ...
  if (configured) {
    return exists(configured)
      ? { level: 'ok', summary: 'PUPPETEER_EXECUTABLE_PATH names an installed browser' }
      : {
        level: 'warn',
  ...
  if (executable && exists(executable)) return { level: 'ok', summary: 'Chrome for Testing is installed' };
  return {
    level: 'info',
...
export function readOpenSkyCredentialsFile(filePath, rootDir = ROOT) {
  ...
  try {
    ...
    return { state: error?.code === 'ENOENT' ? 'missing' : 'invalid', values: {} };
  }
  const values = {
    OPENSKY_CLIENT_ID: String(raw?.clientId ?? raw?.client_id ?? '').trim(),
    OPENSKY_CLIENT_SECRET: String(raw?.clientSecret ?? raw?.client_secret ?? '').trim(),
  };
  return {
    state: values.OPENSKY_CLIENT_ID && values.OPENSKY_CLIENT_SECRET ? 'ok' : 'incomplete',
    values,
  };
}
 ...
 */
export function checkOpenSky({ mode = '', credentials = {}, credentialsFile = null, retired = [] } = {}) {
  const requested = String(mode).trim().toLowerCase();
  if (requested && requested !== 'oauth' && requested !== 'anon') {
    return {
      level: 'warn',
      summary: requested === 'basic' || requested === 'auto'
  ...
  if (requested === 'anon') {
    return { level: 'info', summary: 'anonymous access (OPENSKY_AUTH_MODE=anon); client credentials are not used' };
  }
  if (credentialsFile && credentialsFile.state !== 'ok') {
    ...
    return {
      level: 'warn',
      summary: credentialsFile.state === 'invalid'
  ...
  if (id && secret) {
    return { level: 'ok', summary: 'OAuth client ID and secret found; only OpenSky can tell whether they are valid' };
  }
  if (id || secret) {
    return {
      level: 'warn',
  ...
  return { level: 'info', summary: 'anonymous access; an OAuth API client gets more requests (docs/opensky-auth.md)' };
}
...
function readSetting(name, { environment, rootDir, authoritativeEnvironment }) {
  ...
  if (authoritativeEnvironment && Object.prototype.hasOwnProperty.call(environment, name)) return '';
...
export function resolveCredential(spec, {
  ...
  // before the Keychain.
  if (isConfiguredValue(credentialsFile[spec.name])) return { configured: true, source: 'OpenSky credentials file' };
...
export async function inspectSetup({
  ...
  const setting = (name) => readSetting(name, { environment, rootDir, authoritativeEnvironment });
  ...
  let packageManager = '';
  try {
    ({ packageManager = '' } = JSON.parse(readFileSync(path.join(rootDir, 'package.json'), 'utf8')));
  } catch {
    // hasRequiredDependencies reports an unreadable package.json.
  }
  const pnpmVersion = String(pnpmResult.stdout || '').trim();
  const pnpm = pnpmResult.status === 0
    ? { available: true, version: pnpmVersion, ...checkPnpmVersion(pnpmVersion, packageManager) }
    : { available: false, version: null };

  // The launcher reads the credentials file only in oauth mode.
  const openSkyMode = setting('OPENSKY_AUTH_MODE');
  const credentialsFilePath = includeCredentialsFile && openSkyMode.toLowerCase() !== 'anon'
    ? setting('OPENSKY_CREDENTIALS_FILE')
    : '';
  const credentialsFile = credentialsFilePath
    ? { path: credentialsFilePath, ...readOpenSkyCredentialsFile(credentialsFilePath, rootDir) }
    : null;
  ...
  if (developerChecks) {
    ...
    checks.push(
      { id: 'port', label: `Port ${address.port}`, ...await checkPort(address) },
      { id: 'git-hook', label: 'Git hook', ...checkGitHook({ rootDir, environment }) },
      { id: 'qa-browser', label: 'QA browser', ...await checkQaBrowser({ environment }) },
    );
  }
  checks.push({
    id: 'opensky',
    ...
    ...checkOpenSky({
      ...
      credentials,
      credentialsFile,
      retired: ['OPENSKY_USERNAME', 'OPENSKY_PASSWORD'].filter((name) => isConfiguredValue(setting(name))),
    }),
  });

  return {
    ready: node.level !== 'error'
      && pnpm.available
...
function checkLines(label, { level = 'ok', summary, hint } = {}) {
  const lines = [`[${symbol(level)}] ${summary ? `${label}: ${summary}` : label}`];
  if (hint) lines.push(`       ${hint}`);
  return lines;
}

export function formatSetupReport(report, { readyMessage } = {}) {
  const needsLauncher = Object.values(report.credentials || {})
    .some((credential) => LAUNCHER_SOURCES.has(credential?.source));
  const resolvedReadyMessage = readyMessage || (needsLauncher
