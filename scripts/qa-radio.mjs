  ...
  try {
    ...
    const initial = await page.evaluate(() => {
      ...
      return {
        ...
        tunerCount:
          window.__godsEyeView.styleManager._radioTunerStations.length,
      ...
      try {
        ...
        for (const layerId of ['military-awareness', 'rocket-launches']) {
          ...
          directActivationRollbackFailures.push({
            ...
            toastMessages,
            retryRetained:
              styleManager._contextSessionSnapshot?.enabledLayerIds?.has(
                'local-datacenters',
              ) === true,
          });
        }
        ...
        for (const outcome of ['false', 'reject']) {
          ...
          try {
            ...
            const result = await styleManager._runUserFacingContextAction(
              (notificationToken) =>
                styleManager._deactivateContextForLayerChange({
                  notificationToken,
                }),
    ...
    const highGlobalClusterLabels = await page.evaluate(async () => {
      ...
      const radio = window.__godsEyeView.dataManager.layers.get('radio').module;
      const ellipsoid = viewer.scene.globe.ellipsoid;
      const { getWorldOverlayDiagnostics } = await import(
        '/src/overlays/worldOverlay.js'
      );
      const { distanceFade } = await import(
        '/src/overlays/worldOverlayDraw.js'
    ...
    const absoluteTuner = await page.evaluate(async () => {
      ...
      return {
        ...
        centerExpectedId:
          style._radioTunerStations[Math.floor(centerCoordinate + 0.5)]?.id ||
          null,
    ...
    const tunerDirectRelease = await page.evaluate(async () => {
      ...
      const targetId =
        gev.styleManager._radioTunerStations[targetIndex]?.id || null;
      const rect = slider.getBoundingClientRect();
      const xFor = (index) =>
    ...
    const tunerCommit = await page.evaluate((target) => {
      ...
      return {
        ...
        signatureStable:
          gev.styleManager._radioTunerBandSignature === target.frozenSignature,
        selectedIndex: gev.styleManager._radioTunerStations.findIndex(
    ...
    const viewportTuner = await page.evaluate(async (selectedId) => {
      ...
      const before = {
        ...
        selectedIndex: style._radioTunerStations.findIndex(
    ...
    const nextNeedleBefore = await page.evaluate(() => {
      ...
      const selectedIndex = style._radioTunerStations.findIndex(
        (station) => station.id === selectedId,
      );
      const selectedPoolIndex = style._radioTunerPool.findIndex(
        (station) => station.id === selectedId,
      );
      const expectedPoolIndex =
        (selectedPoolIndex + 1) % style._radioTunerPool.length;
      return {
        signature: style._radioTunerBandSignature,
        selectedIndex,
    ...
    const tunerKeyboard = await page.evaluate(() => {
      ...
      const expectedIndex = Math.min(
        style._radioTunerStations.length - 1,
        startIndex + 1,
      );
