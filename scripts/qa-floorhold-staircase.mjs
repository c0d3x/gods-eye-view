...
for (const [name, schedule] of SCENARIOS) {
  ...
  for (let t = 0; t <= 12000; t += 80) {
    ...
    samples.push([
      t,
      Cesium.Cartographic.fromCartesian(out, Cesium.Ellipsoid.WGS84).height,
    ]);
  }
  ...
  console.log(`\n### ${name}`);
  console.log(
    `  target = ${TARGET.toFixed(2)} m   start = ${START.toFixed(2)} m`,
  );
  console.log(
    `  time-to-surface       = ${settle ? `${settle[0]} ms` : 'NEVER'}`,
  );
  console.log(`  time visibly BURIED   = ${buried} ms`);
  console.log(
    `  time visibly FLOATING = ${floatMs} ms   (worst +${floatMax.toFixed(1)} m)`,
  );
  console.log(
    `  visible steps (>0.5m) = ${steps.length}  (up ${steps.filter(([, d]) => d > 0).length} / down ${steps.filter(([, d]) => d < 0).length})`,
  );
  console.log(`  distinct plateaus     = ${plateaus}`);
  summary.push(
    `  ${name} :: settle=${settle ? settle[0] : 'NEVER'} steps=${steps.length} float=${floatMs}ms(+${floatMax.toFixed(1)}m) buried=${buried}ms`,
  );
}
...
console.log(
  `    grounded, own cell warm       = ${pickRenderAltitudeM({ geoAltM: null, baroAltM: null, onGround: true, surfaceM: JFK_GROUND, geoidN: JFK_GEOID })} m`,
);
console.log(
  `    airborne, baro appears at 0 ft = ${pickRenderAltitudeM({ geoAltM: null, baroAltM: 0, onGround: false, surfaceM: null, geoidN: JFK_GEOID })} m`,
);
console.log(
  `    the source switch alone drops the fix ${(JFK_GROUND - JFK_GEOID).toFixed(1)} m, to below the runway`,
);
...
for (const row of rollRows) {
  ...
  const mark = below(row)
    ? og
      ? '   <-- BELOW THE RUNWAY'
      : '   <-- below the runway (airborne pass-through)'
    : '';
  console.log(
    `  tick ${String(i).padStart(2)}  onGround=${og ? 'T' : 'F'}  render=${h.toFixed(2)}${mark}`,
  );
}
...
console.log(
  `    grounded : ${groundedRows.filter(below).length} / ${groundedRows.length}   <- what the hold owns; must be 0`,
);
console.log(
  `    airborne : ${airborneRows.filter(below).length} / ${airborneRows.length}   <- pass-through by design; accepted transition residual`,
);
const worst = Math.min(JFK_GROUND, ...dipped.map(([, , h]) => h));
console.log(
  `  worst burial          = ${(worst - JFK_GROUND).toFixed(2)} m  (the field's geoid separation, one tick wide)`,
);
