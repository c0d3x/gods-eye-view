...
async function main() {
  ...
  try {
    ...
    console.error(
      `Start it first:  ./scripts/dev-fresh.sh   (or:  pnpm run dev)`,
    );
    process.exit(2);
  }
  ...
  try {
    ...
    const dfSetup = await evalPage(async () => {
      ...
      window.__dfCandidates = [
        ...new Set([...seen.reverse(), '/src/data/groundFloor.js']),
      ];
      return { candidates: window.__dfCandidates.length };
    });
    record(
      'display-floor: groundFloor module URL candidates found',
    ...
    const dfIdentity = await evalPage(
      async (lat, lon, seedM) => {
        ...
        for (let i = 0; i < window.__dfCandidates.length; i++) {
          ...
          let gf;
          try {
            gf = await import(/* @vite-ignore */ url);
          } catch {
    ...
    if (!identityOk) {
      ...
      const dfTracked = await evalPage(async () => {
        ...
        try {
          ...
          if (!zoomedOut)
            return { error: 'tracked entity has no position (zoomed out)' };
      ...
      const dfHold = await evalPage(async () => {
        ...
        const urls = [
          ...new Set([
            ...performance
              .getEntriesByType('resource')
              .map((e) => e.name)
              .filter((n) => /\/src\/data\/flights\.js(\?|$)/.test(n))
              .reverse(),
            '/src/data/flights.js',
          ]),
        ];
        for (const url of urls) {
          ...
          if (
            mod?.default === fl &&
            typeof mod._driveFleetModelHandoffForTest === 'function'
          ) {
            ns = mod;
            break;
          }
        ...
        if (!ns)
          return {
            skipped: `the app's own flights module was not reachable (tried ${urls.length})`,
          };
