 ...
 *
 * Run:  node scripts/qa-traffic.mjs --url <app-url>
...
async function main() {
  ...
  const statusBefore = await fetch(`${APP_URL}/api/tomtom/status`)
    .then((r) => r.json())
    .catch(() => null);
  ...
  try {
    ...
    {
      const statusAfter = await fetch(`${APP_URL}/api/tomtom/status`).then(
        (r) => r.json(),
      );
