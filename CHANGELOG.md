...
- `src/ui.js` is being split into modules under `src/ui/`, one panel at a
  ...
  shares, the Location bar, camera navigation and the visual styles. The
  cockpit signal tests now drive the real Cockpit controller instead of
  code cut out of `ui.js` as text.
