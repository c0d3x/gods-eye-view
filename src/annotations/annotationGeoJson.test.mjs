...
test('point (pin/highlight/label) round-trips, height preserved', () => {
  const pin = {
    ...
    color: 'primary',
    ttlMs: null,
    anchor: { lon: -77.9, lat: 34.27, height: 12 },
    to: null,
    ring: null,
  };
...
test('route path round-trips with mode/distance/duration', () => {
  const path = [
    { lon: -122.4, lat: 37.7, height: 5 },
    { lon: -122.39, lat: 37.71, height: 6 },
    { lon: -122.38, lat: 37.72, height: 7 },
  ];
  const route = {
    type: 'route',
    id: 'anno-6',
    ...
    color: 'primary',
    ttlMs: null,
    anchor: { lon: -122.4, lat: 37.7, height: 5 },
    to: null,
    ring: null,
    path,
    mode: 'foot',
...
test('arrow round-trips (anchor -> to)', () => {
  const arrow = {
    ...
    color: 'primary',
    ttlMs: null,
    anchor: { lon: -122.43, lat: 37.8, height: 3 },
    to: { lon: -122.4, lat: 37.81, height: 4 },
    ring: null,
  };
...
test('malformed input fails CLOSED (returns null / skips)', () => {
  ...
  assert.equal(
    annotationToFeature({ type: 'blob', anchor: { lon: 0, lat: 0 } }),
    null,
  );
  ...
  assert.deepEqual(annotationsToFeatureCollection('nope'), {
