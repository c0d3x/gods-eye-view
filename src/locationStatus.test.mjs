...
test('a preset city reports its framed POI', () => {
  assert.deepEqual(
    locationMiniStatus({
      city: NEW_YORK,
      currentPoi: { name: 'Empire State Building' },
    }),
...
test('a preset city with no framed POI falls back to its first POI', () => {
  assert.deepEqual(locationMiniStatus({ city: NEW_YORK }), {
    city: '📍 New York',
...
test('a free-text search reports the destination, never the empty placeholder', () => {
  ...
  assert.deepEqual(locationMiniStatus({ searchedLabel: 'Tokyo, Japan' }), {
    city: '📍 Tokyo',
    poi: 'Japan',
  });
  assert.deepEqual(
...
test('a single-segment geocode says it was searched rather than inventing context', () => {
  assert.deepEqual(locationMiniStatus({ searchedLabel: 'Japan' }), {
    city: '📍 Japan',
    poi: 'Searched location',
  });
});
...
test('nothing selected keeps the honest empty placeholder', () => {
  ...
  assert.deepEqual(
    locationMiniStatus({ city: null, searchedLabel: '' }),
    empty,
  );
