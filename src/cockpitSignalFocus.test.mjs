...
function fixture(t) {
  ...
  const byId = new Map([
    ['cockpit-signal-list', signalList],
    ['cockpit-signal-toggle', signalToggle],
  ]);
  document.getElementById = (id) => byId.get(id) ?? null;
  document.querySelectorAll = (selector) =>
    selector === '[data-cockpit-brief-index]' ? briefTabs : [];
  ...
  const selected = [];
  t.mock.method(militaryAwarenessLayer, 'focusTarget', (...args) => {
    selected.push(args);
  });

  const listenable = { addEventListener: () => () => {} };
  const controller = new CockpitViewController({
    scene: { preUpdate: listenable },
    trackedEntityChanged: listenable,
  });
