...
function stubCamera() {
  ...
  return {
    ...
    cancelFlight() {
      calls.cancelFlight += 1;
    },
    flyToBoundingSphere(sphere, options) {
      calls.flights.push({ sphere, options });
    },
  };
}
...
test('a focus request carries the clicked target to any registered listener', () => {
  ...
  const seen = [];
  const dispose = registerWorldFocusRequestListener(target, (event) =>
    seen.push(event.detail),
  );
  assert.equal(
    requestWorldFocus(
      { kind: 'vessel', position: POSITION, id: '123' },
      target,
    ),
    true,
  );
...
test('requests without a kind or a position are dropped, never dispatched', () => {
  const target = new EventTarget();
  let count = 0;
  target.addEventListener(WORLD_FOCUS_REQUEST_EVENT, () => {
    count += 1;
  });
...
test('an unflyable request never reaches the release policy', () => {
  ...
  for (const detail of unflyable) {
    assert.equal(
      isValidWorldFocusTarget(detail),
      false,
    ...
    assert.equal(
      policyRuns,
      0,
      `the policy must not run for ${JSON.stringify(detail)}`,
    );
    // Nor may such a request be dispatched in the first place.
    assert.equal(requestWorldFocus(detail, new EventTarget()), false);
  }
  assert.equal(
    isValidWorldFocusTarget({ kind: 'vessel', id: '123', position: POSITION }),
    true,
  );
  assert.equal(
    isValidWorldFocusTarget({ kind: 'fire', id: 'fire-1', position: POSITION }),
    true,
  );
});
...
test('the transfer flight supersedes any flight in progress and keeps the operator heading', () => {
  ...
  assert.equal(
    flyToWorldTarget(
      { camera },
      { kind: 'vessel', id: '123', position: POSITION },
    ),
    true,
  );
  assert.equal(
    camera.calls.cancelFlight,
    1,
    'a prior flight must be cancelled, not queued',
  );
...
test('fires frame wider than vessels — a fire is read by its surroundings', () => {
  ...
  flyToWorldTarget(
    { camera },
    { kind: 'fire', id: 'fire-1', position: POSITION },
  );
  const { sphere, options } = camera.calls.flights[0];
  ...
  assert.ok(
    WORLD_FOCUS_FRAMING.fire.rangeM > WORLD_FOCUS_FRAMING.vessel.rangeM,
  );
  assert.ok(
    WORLD_FOCUS_FRAMING.fire.radiusM > WORLD_FOCUS_FRAMING.vessel.radiusM,
  );
});
...
test('every framing is a real oblique standoff, not a nadir or an inside-out sphere', () => {
  ...
  for (const kind of kinds) {
    ...
    assert.ok(
      framing.pitchDeg < 0,
      `${kind}: pitch must look down, got ${framing.pitchDeg}`,
    );
    assert.ok(
      framing.pitchDeg > -80,
      `${kind}: pitch must stay oblique, got ${framing.pitchDeg}`,
    );
    ...
    assert.ok(
      framing.rangeM <= 10000,
      `${kind}: range must stay close-in, got ${framing.rangeM}`,
    );
  }
  // Exact shipped values — a silent retune must show up as a failing test.
  assert.deepEqual(
    { ...WORLD_FOCUS_FRAMING.vessel },
    { radiusM: 150, rangeM: 1200, pitchDeg: -30 },
  );
  assert.deepEqual(
    { ...WORLD_FOCUS_FRAMING.fire },
    { radiusM: 400, rangeM: 3000, pitchDeg: -35 },
  );
});
...
test('unknown kinds and missing viewers issue no flight', () => {
  const camera = stubCamera();
  assert.equal(
    flyToWorldTarget({ camera }, { kind: 'plane', position: POSITION }),
    false,
  );
  assert.equal(flyToWorldTarget({ camera }, { kind: 'vessel' }), false);
  assert.equal(
    flyToWorldTarget(null, { kind: 'vessel', position: POSITION }),
    false,
  );
