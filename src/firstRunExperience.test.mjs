...
test('the key handler refuses to act for a card that is not really on screen', () => {
  ...
  assert.match(
    module,
    /const\s*isTopmost\s*=\s*\(,?\s*\)\s*=>\s*root\s*\.isConnected/,
  );
  assert.match(
    module,
    /&&\s*root\s*\.getClientRects\(,?\s*\)\s*\.length\s*>\s*0\s*&&\s*!coveredByOverlay\(,?\s*\);/,
  );
...
test('one ESC does one thing — the radio disclosure stops the launcher outright', () => {
  ...
  const radioEsc = ui.slice(
    looseIndexOf(ui, "if (event.key !== 'Escape' || !this._contextRadioDock"),
  );
...
test('a surface class that never clears is an ACCEPTED no-show, not a timer', () => {
  ...
  const state = fs.readFileSync(
    new URL('../docs/current-state/first-run-and-sharing.md', import.meta.url),
    'utf8',
  );
...
test('the launcher yields on engage and waits when a surface is already up', () => {
  ...
  assert.match(
    module,
    /attributes:\s*true,\s*attributeFilter:\s*\[\s*'class',?\s*\]/,
  );
...
test('the tile is the FULLY CONFIGURED experience: quakes and fires together', () => {
  ...
  // Keyless, both surfaces tell the truth: the LAYER ROW reads KEY REQUIRED,
  // and the global chip ends the load as KEY REQUIRED rather than LOAD FAILED.
  // The note stays where the next editor will read it, so a keyless visitor's
  // KEY REQUIRED is not re-discovered as a launcher bug.
  const module = fs.readFileSync(
    new URL('./firstRunExperience.js', import.meta.url),
    'utf8',
  );
...
test('no mission writes a preference the visitor did not choose by picking it', () => {
  ...
  assert.match(
    contextPath,
    /result\s*\?\s*\.ok[\s\S]*?setPanelCollapsed\s*\?\.\(\s*'global-context-panel',\s*false,\s*\{\s*explicit:\s*true,?\s*\},?\s*\)/,
  );
});
...
test('markup, startup ordering and accessibility remain pinned', () => {
  ...
  const startup = fs.readFileSync(
    new URL('./editions/local/startupChrome.js', import.meta.url),
    'utf8',
  );
  const css = fs.readFileSync(new URL('../style.css', import.meta.url), 'utf8');
...
function mountLauncher() {
  ...
  page.root = element({
    ...
    setAttribute(name, value) {
      if (name === 'aria-hidden')
        page.focusWhenHidden = documentRef.activeElement;
...
test('focus leaves the launcher before it is hidden, whichever way it closes', () => {
  ...
  assert.equal(
    closed.documentRef.activeElement,
    closed.buttons[0],
    'reveal focuses the first mission',
  );
...
test('the voice TOOL SCHEMA is unchanged — the mission mapping is instructions only', () => {
  ...
  assert.equal(
    ...
    28,
    'tool count drifted from the pinned release schema',
  );
  assert.equal(
    schema.length,
    26121,
    'tool schema length drifted from the pinned release schema',
  );
  assert.equal(
...
test('every layer a mission drives is already in the shipped set_layer_visibility enum', () => {
  const tool = GEV_REALTIME_TOOLS.find(
    (entry) => entry.name === 'set_layer_visibility',
  );
