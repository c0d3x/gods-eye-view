...
test('dev-fresh gives explicit or dotenv Google configuration precedence over Keychain', async () => {
  const source = await fs.readFile(
    new URL('../scripts/dev-fresh.sh', import.meta.url),
    'utf8',
  );
  const precedence = source.indexOf('if [[ -n "${GOOGLE_MAPS_API_KEY_ENV}" ]]');
  const keychainFallback = source.indexOf(
    'elif [[ -n "${GOOGLE_MAPS_API_KEY_KEYCHAIN}" ]]',
  );
...
test('dev-fresh passes names-only boot provenance before resolving file fallbacks', async () => {
  ...
  const dotenvResolution = source.indexOf(
    'GOOGLE_MAPS_API_KEY_ENV="${GOOGLE_MAPS_API_KEY:-}"',
  );
  assert.ok(
    capture >= 0 && capture < dotenvResolution,
    'parent-shell provenance must be captured first',
  );
  for (const name of [
    'GOOGLE_MAPS_API_KEY',
    'CESIUM_ION_TOKEN',
    ...
    assert.match(
      source,
      new RegExp(`KEY_SETUP_EXTERNAL_KEYS\\+=\\(${name}\\)`),
    );
  }
...
bashTest(
  'dev-fresh reports the real keyless startup map and fallback',
  async () => {
    ...
    assert.ok(
      start >= 0 && end > start,
      'startup-map status block not found in dev-fresh.sh',
    );
    const block = script.slice(start, end + 3);
    assert.doesNotMatch(
      block,
      /Startup map: OpenStreetMap with keyless terrain/,
    );
...
bashTest(
  'the external-keys provenance block survives set -u keyless and joins names when keys are exported',
  async () => {
    const script = await fs.readFile(
      new URL('../scripts/dev-fresh.sh', import.meta.url),
      'utf8',
    ...
    const end = script.indexOf('\n', csvAt);
    assert.ok(
      start > 0 && csvAt > start && end > csvAt,
      'provenance block not found in dev-fresh.sh',
    );
    ...
    const keyless = await run('bash', ['-c', probe], {
