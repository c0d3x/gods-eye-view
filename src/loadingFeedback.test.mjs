...
test('share-follow failures use the universal top-center status instead of the bottom toast', () => {
  ...
  assert.match(
    handler,
    /startupCover\s*\.addEventListener\(\s*'transitionend',\s*showOnce,\s*\{\s*once:\s*true,?\s*\},?\s*\)/,
  );
  ...
  assert.match(
    handler,
    /result\s*\.classification\s*===\s*'followed'\s*\|\|\s*result\s*\.classification\s*===\s*'cancelled'/,
  );
});
...
test('a layer that only lacks its key ends the load as KEY REQUIRED, not LOAD FAILED', () => {
  ...
  assert.equal(
    state.hideAt,
    300 + LOADING_FAILURE_DWELL_MS,
    'it stays long enough to act on',
  );
  ...
  const notice = createGlobalStatusNotice('Recording', 300, {
    state: 'acquiring',
    persistent: true,
  });
  assert.equal(
    presentGlobalLoadingStatus(notice, state, keyless, 300).state,
    'needs-key',
  );
});
...
test('a real failure in the same load still reads LOAD FAILED', () => {
  ...
  const settled = aggregateLayerLoading([
    ...
    },
    {
      ...
      enabled: true,
      lifecycleState: 'enabled',
      stats: { error: 'USGS network error' },
    },
  ]);
  state = reduceLoadingFeedback(state, settled, 300);
  assert.equal(state.terminal, 'error');
  assert.equal(
    presentLoadingFeedback(state, settled, 300).label,
    'LOAD FAILED',
  );
});
...
test('the KEY REQUIRED banner keeps its detail and offers an ADD KEY button', () => {
  ...
  assert.match(
    html,
    /<button id="global-loading-action" type="button" hidden>Add key<\/button>/,
  );
  assert.match(
    css,
    /#global-loading-status\[data-state='needs-key'\] \{[^}]*pointer-events: auto;/,
  );
  // Unlike the other terminal states, its detail line (the key's name) shows.
  assert.doesNotMatch(
    css,
    /\[data-state='needs-key'\][^{}]*#global-loading-detail\s*\{\s*display:\s*none/,
  );
});
