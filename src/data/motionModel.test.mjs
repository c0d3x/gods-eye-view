...
test('courseBetweenCartesians: due north ≈ 0°, due east ≈ 90°', () => {
  ...
  const east = cart(
    0,
    1000 / (111320 * Math.cos((AUSTIN.lat * Math.PI) / 180)),
  );
...
test('turn-rate guard: hover fix-track jitter manufactures NO turn rate; a real slow turn still does', () => {
  ...
  const mixed = [
    { tSec: 0, trackDeg: 0, speedMps: 1 },
    { tSec: 15, trackDeg: 90, speedMps: 1 }, // noise interval (skipped)
    { tSec: 30, trackDeg: 90, speedMps: 100 },
    { tSec: 45, trackDeg: 120, speedMps: 100 }, // real 2°/s
  ];
...
test('repeated kinematics apply only from a forward synthetic fix', () => {
  ...
  const originalPosition = Cesium.Cartesian3.fromDegrees(
    -97.7431,
    30.2672,
    9000,
  );
  ...
  assert.ok(
    Math.abs(
      Cesium.Cartesian3.distance(newest.position, synthetic.position) - 1000,
    ) < 0.5,
  );
});
...
test('liftRepeatedGroundFix raises only grounded history and preserves the stored coordinates', () => {
  ...
  assert.ok(
    Math.abs(Cesium.Cartographic.fromCartesian(newest.position).height - 320) <
      1e-5,
  );
});
...
test('projectGroundArcLatLon curves away from the tangent under a sustained turn', () => {
  ...
  assert.ok(
    arc.lon > tangent.lon + 0.001,
    `arc must swing east: ${arc.lon} vs ${tangent.lon}`,
  );
  assert.ok(
    arc.lat < tangent.lat - 0.001,
    `arc must fall short north: ${arc.lat} vs ${tangent.lat}`,
  );
});
...
test('projectGroundArcLatLon turns left for a negative rate', () => {
  ...
  assert.ok(
    left.lon < -97.66 && right.lon > -97.66,
    `left ${left.lon} right ${right.lon}`,
  );
});

test('projectGroundArcLatLon returns the origin for a zero-length leg', () => {
  assert.deepEqual(projectGroundArcLatLon(30.2, -97.66, 45, 10, 0, 0), {
    lat: 30.2,
    lon: -97.66,
  });
  assert.deepEqual(projectGroundArcLatLon(30.2, -97.66, 45, 0, 0, 30), {
    lat: 30.2,
    lon: -97.66,
  });
});
...
test('projectGroundArcLatLon returns the origin for non-finite inputs', () => {
  assert.deepEqual(projectGroundArcLatLon(30.2, -97.66, NaN, 10, 0, 30), {
    lat: 30.2,
    lon: -97.66,
  });
  assert.deepEqual(projectGroundArcLatLon(30.2, -97.66, 45, 10, 0, NaN), {
    lat: 30.2,
    lon: -97.66,
  });
});
...
test('corridorPathLatLon leads FORWARD, past the fix, while coasting', () => {
  ...
  assert.ok(end.lat > 30.2, `must lead the contact, got ${end.lat}`);
  assert.ok(end.lat > 30.197, 'never the backtrail behind the stale fix');
  assert.ok(
    Math.abs(end.lat - (30.2 + 600 / 111320)) < 1e-5,
    `end lat ${end.lat}`,
  );
});
...
test('corridorPathLatLon samples the ARC, not a chord, for a turning coaster', () => {
  ...
  assert.ok(
    Math.abs(mid.lon - chordLon) > 1e-4,
...
test('corridorPathLatLon falls back to projection when the fix is unusable', () => {
  ...
  assert.ok(
    path[path.length - 1].lon > -97.66,
    'still produces a usable endpoint',
  );
});
