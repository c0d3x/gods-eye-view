...
test('awareness formatters preserve valid zeroes and replace missing or invalid values', () => {
  ...
  assert.equal(
    formatAwarenessLabel({ callsign: ' UAL649 ', id: 'fallback' }),
    'UAL649',
  );
  assert.equal(
    formatAwarenessLabel({ name: '', label: 'Mapped site' }),
    'Mapped site',
  );
...
test('awareness cohorts sort nearest-first and do not turn empty data into out of range', () => {
  const result = summarizeAwarenessCohort([
    { id: 'far', distance: 9000 },
    { id: 'near', distanceM: 1000 },
  ]);
  assert.equal(result.relationship, 'NEARBY');
  assert.deepEqual(
    result.nearest.map((item) => item.id),
    ['near', 'far'],
  );
...
test('unavailable and stale feeds remain unknown', () => {
  ...
  assert.equal(
    summarizeAwarenessCohort([], { available: false }).reason,
    'feed unavailable',
  );
  assert.equal(
    summarizeAwarenessCohort([], { stale: true }).reason,
    'feed stale',
  );
});
...
test('navigation prioritizes nearby targets from the selected cohort', () => {
  const cohorts = [
    {
      ...
      summary: {
        nearest: [
          { icao24: 'a1', distanceM: 1000 },
          { icao24: 'a2', distanceM: 2000 },
        ],
      },
    },
    {
      id: 'military',
      summary: { nearest: [{ icao24: 'm1', distanceM: 500 }] },
    },
  ];
  const targets = getAwarenessNavigationTargets(
    cohorts,
    { layerId: 'flights', id: 'subject' },
    ['flights:a1'],
  );
  ...
  assert.deepEqual(
    targets.map((target) => `${target.layerId}:${target.id}`),
    ['flights:a2', 'military:m1', 'flights:a1'],
  );
});
...
test('awareness cycling excludes satellites, which retain independent tracking', () => {
  const cohorts = [
    ...
    {
      id: 'ais-live-vessels',
      summary: { nearest: [{ mmsi: 'v1', distanceM: 1000 }] },
    },
  ];
...
test('expanding proximity search stops at its global bound when feeds are empty', () => {
  const searched = [];
  const result = findByDoublingRadius(
    (radiusM) => {
      searched.push(radiusM);
      return null;
    },
