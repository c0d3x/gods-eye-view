...
test('layer rows show names and icons as text, never as markup', async () => {
  ...
  try {
    ...
    assert.equal(
      container.querySelector('.data-name').textContent,
...
test('a timed refresh that settles after the layer is off adds nothing', async (t) => {
  ...
  mgr.register({
    ...
    getStats() {
      return { count: entities.length, lastUpdate: null };
    },
  });
  ...
  try {
    ...
    assert.ok(
      timedSignal instanceof AbortSignal,
      'timed refreshes get a signal',
    );
    ...
    assert.equal(
      timedSignal.aborted,
      true,
      'turning the layer off cancels the refresh',
    );
    releaseRefresh();
...
test('rapid on/off toggling settles in one consistent state', async (t) => {
  ...
  try {
    ...
    await Promise.all(
      Array.from({ length: 5 }, () =>
        mgr.toggle('toggled', { origin: 'user' }),
      ),
    );
