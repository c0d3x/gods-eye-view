...
test('fixture decode: every trafficLevel is within [0, 1]', () => {
  ...
  for (const s of segments) {
    assert.ok(
      Number.isFinite(s.trafficLevel),
      `non-finite level: ${s.trafficLevel}`,
    );
    assert.ok(
      s.trafficLevel >= 0 && s.trafficLevel <= 1,
      `level out of range: ${s.trafficLevel}`,
    );
  }
});
...
test('fixture decode: congestion exists (at least one trafficLevel < 1)', () => {
  ...
  assert.ok(
    segments.some((s) => s.trafficLevel < 1),
    'no congested segment found',
  );
});
...
test('fixture decode: segment shape is {coords, trafficLevel, roadType, closure}', () => {
  ...
  assert.ok(
    segments.some((s) => s.closure === true),
    'expected at least one closure in fixture',
  );
});

test('decode of a non-MVT buffer returns [] (defensive)', () => {
  assert.deepEqual(
    decodeFlowTile(Buffer.from('not a protobuf tile'), 12, 935, 1686),
    [],
  );
});
...
test('fetchFlowForBounds: non-OK tile responses reject when nothing succeeds', async () => {
  ...
  const restore = stubFetch(
    async () =>
      new Response(JSON.stringify({ error: 'no_key' }), { status: 503 }),
  );
  try {
