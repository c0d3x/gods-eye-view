...
test('every ingested CelesTrak group resolves to a real class', () => {
  ...
  for (const group of INGESTED_GROUPS) {
    ...
    assert.ok(
      SATELLITE_CLASSES[klass],
      `${group} maps to an unknown class ${klass}`,
    );
    assert.ok(
      SATELLITE_CLASS_ORDER.includes(klass),
      `${klass} is missing from the legend order`,
    );
  }
});
...
test('the three GNSS constellations share one NAV color and split by subtype', () => {
  ...
  assert.equal(
    colors.size,
    1,
    'GPS, GLONASS and Galileo must read as one NAV family',
  );
...
test('class labels name the type, and the ISS names itself', () => {
  ...
  assert.equal(
    satelliteClassLabel('stations', { isIss: true }),
    'STATION · ISS',
  );
});
...
test('the ISS is a STATION whichever group a partial outage ingested it from', () => {
  ...
  assert.equal(
    satelliteClassLabel(undefined, { isIss: true }),
    'STATION · ISS',
  );
...
test('class colors are distinct, valid, and never borrow the military amber', () => {
  ...
  for (const klass of SATELLITE_CLASS_ORDER) {
    ...
    const amberish =
      hueOf(color) >= 35 && hueOf(color) <= 50 && saturationOf(color) > 0.45;
    assert.equal(
      amberish,
      false,
      `${klass} (${color}) sits in the military amber band`,
    );
  }
});
...
test('the dense shell stays dimmer than the class it sits among', () => {
  ...
  assert.ok(
    luma(SATELLITE_CLASSES.visual.color) - luma(SATELLITE_CLASSES.comms.color) >
      0.2,
    'COMMS must stay well below VISUAL in luminance',
  );
  assert.ok(
    luma(SATELLITE_CLASSES.station.color) >
      luma(SATELLITE_CLASSES.visual.color),
...
test('the tally files the ISS under STATION even when a partial outage moved it', () => {
  ...
  assert.equal(
    legend.some((row) => row.klass === 'station'),
    true,
  ...
  assert.deepEqual(
    { ...tallySatelliteClasses(['visual', 'visual']) },
    { visual: 2 },
  );
  assert.equal(
    satelliteClassOf('visual', { isIss: true }).klass,
    'station',
...
test('the legend lists present classes in order and omits absent ones', () => {
  const legend = satelliteClassLegend(
    tallySatelliteClasses(['geo', 'gps-ops', 'stations']),
  );
  assert.deepEqual(
    legend.map((row) => row.klass),
    ['station', 'nav', 'geo'],
  ...
  assert.equal(
    legend.some((row) => row.klass === 'comms'),
    false,
  );
  ...
  assert.ok(
    row.blurb.length > 0,
    'every legend row carries a plain-language gloss',
  );
});

test('the legend drops zero and negative counts', () => {
  assert.deepEqual(
    satelliteClassLegend({ nav: 0, geo: -1, station: 2 }).map((r) => r.klass),
    ['station'],
  );
...
test('the DENSE row chip is stateless and declares the params to apply', () => {
  ...
  try {
    _setDenseCatalogStateForTest({});
    const chip = satellitesLayer
      .getRowControls()
      .chips.find((entry) => entry.id === 'catalog');
    ...
    assert.deepEqual(
      chip.params,
      { catalog: 'dense' },
      'clicking from core requests dense',
    );
...
test('DENSE reports loading, then ACTIVE only once the points exist', async () => {
  ...
  try {
    ...
    _setDenseCatalogStateForTest({});
    satellitesLayer.setRowControlsListener(() =>
      refreshes.push(satellitesLayer.getRowControls()),
    );
    ...
    assert.equal(
      settled.active,
      true,
      'active once the dense points are on screen',
    );
    ...
    assert.deepEqual(
      settled.params,
      { catalog: 'core' },
      'a settled chip toggles back off',
    );
    ...
    assert.ok(
      refreshes.length >= 2,
      'load start and load completion each pushed a refresh',
    );
...
test('a failed DENSE load reverts the mode rather than leaving an active chip', async () => {
  ...
  try {
    ...
    assert.equal(
      settled.active,
      false,
      'a 502 must never present as a live dense catalog',
    );
    assert.equal(settled.state, 'error');
    assert.equal(settled.label, 'DENSE ✕');
    assert.match(settled.title, /502/, 'the chip explains why');
    assert.equal(
      satellitesLayer.getParams().catalog,
      'core',
      'the mode reverts to reality',
    );
    ...
    assert.doesNotMatch(
      recovered.title,
      /502/,
      'the stale failure reason is gone',
    );
  } finally {
    console.warn = warn;
    globalThis.fetch = originalFetch;
  ...
  try {
    for (const [name, body] of Object.entries(bodies)) {
      ...
      assert.equal(
        chip.active,
        false,
        `${name}: never active with zero dense points`,
      );
    ...
    satellitesLayer.setParams({ catalog: 'dense' });
    assert.equal(
      (await settleChip()).state,
      'active',
      'a real feed still loads',
    );
    _clearDenseCatalogStateForTest();
  } finally {
    console.warn = warn;
...
test('an explicit return to core clears a failure the user never caused', async () => {
  ...
  try {
    ...
    assert.equal(
      satellitesLayer.getParams().catalog,
      'core',
      'the failure already reverted it',
    );
    ...
    assert.equal(
      restored.state,
      'idle',
      'the error does not survive the restore',
    );
...
test('a real stations-feed outage keeps STATION in the legend, matching the card', async () => {
  ...
  try {
    ...
    const legend = satellitesLayer.getRowControls().legend;
    assert.equal(
      legend.find((row) => row.klass === 'station')?.count,
      1,
      'STATION survives the outage because the ISS is counted there',
    );
    assert.equal(
      legend.some((row) => row.klass === 'visual'),
      false,
      'and it is not also counted under VISUAL',
    );
    assert.equal(
      satelliteClassLabel('visual', { isIss: true }),
      'STATION · ISS',
...
test('a catalog rebuild refreshes the detection overlay class strings', async () => {
  ...
  try {
    ...
    await satellitesLayer.update(viewer);
    const first = satellitesLayer
      .getDetectableObjects()
      .find((o) => o.sourceId === 33333);
    assert.ok(first, 'the seeded satellite is collectable');
    assert.equal(first.klass, 'GEO');

    homeGroup = 'visual';
    await satellitesLayer.update(viewer);
    const second = satellitesLayer
      .getDetectableObjects()
      .find((o) => o.sourceId === 33333);
    assert.equal(
      second.klass,
      'VISUAL',
      'the cached record does not outlive its catalog',
    );
  } finally {
    console.log = log;
    console.warn = warn;
