...
test('a registration matches in the form the operator says it', () => {
  ...
  assert.equal(
    rankContactMatch({ query: 'GABCD', ...hyphenated }),
    CONTACT_MATCH_TIER.REGISTRATION_EXACT,
  );
  assert.equal(
    rankContactMatch({ query: 'g-abcd', ...hyphenated }),
    CONTACT_MATCH_TIER.REGISTRATION_EXACT,
  );
  const military = { hex: 'ae7f01', registration: '05-8152' };
  assert.equal(
    rankContactMatch({ query: '058152', ...military }),
    CONTACT_MATCH_TIER.REGISTRATION_EXACT,
  );
  assert.equal(
    rankContactMatch({ query: '05-8152', ...military }),
    CONTACT_MATCH_TIER.REGISTRATION_EXACT,
  );
});
...
test('a civilian N-number resolves in every form it is written or spoken', () => {
  ...
  assert.equal(
    rankContactMatch({ query: 'N123AB', ...civilian }),
    CONTACT_MATCH_TIER.REGISTRATION_EXACT,
  );
  assert.equal(
    rankContactMatch({ query: 'n-123ab', ...civilian }),
    CONTACT_MATCH_TIER.REGISTRATION_EXACT,
  );
  assert.equal(
    rankContactMatch({ query: 'n123', ...civilian }),
    CONTACT_MATCH_TIER.REGISTRATION_PREFIX,
  );
  assert.equal(
    rankContactMatch({ query: '123ab', ...civilian }),
...
test('nothing matches an empty or unknown query', () => {
  assert.equal(
    rankContactMatch({ query: '', hex: 'a1', callsign: 'SWA123' }),
  ...
  assert.equal(
    rankContactMatch({ query: 'zzz', hex: 'a1', callsign: 'SWA123' }),
    CONTACT_MATCH_TIER.NONE,
  );
  // A contact with no identity at all cannot be matched by a non-empty query.
  assert.equal(
    rankContactMatch({ query: 'zzz', hex: '' }),
    CONTACT_MATCH_TIER.NONE,
  );
});
...
test('a same-tier tie resolves the same way every time', () => {
  ...
  assert.equal(
    contactMatchWins(a, b),
    false,
    'and the higher hex never displaces it',
  );
});
...
test('a stronger tier always beats a weaker one, whichever arrived first', () => {
  ...
  assert.equal(
    contactMatchWins(exact, prefix),
    true,
    'tier outranks the tiebreak key',
  );
...
test('a non-match never becomes the best match', () => {
  assert.equal(
    contactMatchWins({ tier: CONTACT_MATCH_TIER.NONE, id: 'aaa111' }, null),
    false,
  );
  assert.equal(
    contactMatchWins(
