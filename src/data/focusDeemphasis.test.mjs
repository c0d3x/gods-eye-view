...
test('focus decision dims a farther sprite inside and preserves one outside', () => {
  assert.equal(
    focusTargetEmphasis({ x: 50, y: 50 }, 1200, target, params),
    0.25,
  );
...
test('focus decision keeps nearer behavior tunable across allow, dim, and partial', () => {
  assert.equal(
    focusTargetEmphasis({ x: 50, y: 50 }, 800, target, {
      ...params,
...
test('focus decision applies padding and hysteresis at the boundary', () => {
  assert.equal(
    focusTargetEmphasis({ x: 64, y: 50 }, 1200, target, {
      ...params,
      paddingPx: 3,
    }),
    1,
  ...
  assert.equal(
    focusTargetEmphasis({ x: 63, y: 50 }, 1200, target, {
      ...params,
      paddingPx: 3,
    }),
    0.25,
...
test('focus overlap includes the ambient sprite own rendered extent', () => {
  assert.equal(
    focusTargetEmphasis({ x: 68, y: 50 }, 1200, target, params, false, 8, 8),
    0.25,
  );
  assert.equal(
    focusTargetEmphasis({ x: 68, y: 50 }, 1200, target, params, false, 2, 2),
    1,
  );
});
...
test('focus decision never falls below the configured floor', () => {
  assert.equal(
    focusTargetEmphasis({ x: 50, y: 50 }, 1200, target, {
      ...params,
      dimFloor: 0.41,
    }),
    0.41,
...
test('smoothing converges over configured attack and release instead of snapping', () => {
  ...
  assert.ok(
    releaseMid > 0.25 && releaseMid < 1,
    `release midpoint=${releaseMid}`,
  );
...
test('default attack yields at least four distinct 80 ms sampled values', () => {
  const samples = [0, 80, 160, 240, 320].map((elapsedMs) =>
    smoothFocusEmphasis(1, 0.25, elapsedMs, params),
  );
  assert.ok(
    new Set(samples.slice(1)).size >= 4,
...
test('per-sprite state attacks, restores after focus clears, and settles at one', () => {
  ...
  assert.equal(
    advanceSpriteFocus(sprite, { ...input, nowMs: 300 }).factor,
    0.25,
  );
  assert.equal(
    advanceSpriteFocus(sprite, { ...input, target: null, nowMs: 300 }).factor,
    0.25,
  );
  ...
  assert.equal(
    advanceSpriteFocus(sprite, { ...input, target: null, nowMs: 900 }).factor,
    1,
  );
});
...
test('distance chatter reanchors from current progress and converges toward the majority state', () => {
  ...
  const factors = distances.map(
    (cameraDistance, index) =>
      advanceSpriteFocus(sprite, {
        screenPosition: { x: 50, y: 50 },
        cameraDistance,
        target,
  ...
  for (let index = 1; index < factors.length; index += 1) {
    assert.ok(
      factors[index] <= factors[index - 1] + 1e-12,
  ...
  const dimSprite = {};
  const dimFactors = [0, 80, 160, 240].map(
    (nowMs, index) =>
      advanceSpriteFocus(dimSprite, {
        screenPosition: { x: 50, y: 50 },
...
test('alternating desired state reanchors from sampled progress and converges to a stable cycle', () => {
  ...
  assert.ok(
    lateEvenDelta < earlyEvenDelta,
    `even factors=${factors.join(',')}`,
  );
...
test('advanceSpriteFocus returns its documented module singleton', () => {
  ...
  assert.equal(
    first.factor,
    params.dimFloor,
    'the prior reference reflects the next call',
  );
});
...
test('evidence clock produces identical alpha sequences across repeated captures', () => {
  ...
  const capture = (productionClockBase, productionClockStep) => {
    ...
    const sequence = sequenceOffsets.map((offset, index) => {
      if (index > 0)
        advanceFocusEvidenceNowMs(offset - sequenceOffsets[index - 1]);
      const nowMs = focusNowMs(
        productionClockBase + index * productionClockStep,
      );
