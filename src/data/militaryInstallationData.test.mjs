...
test('normalizes allowed OSM installation features and deduplicates ids', () => {
  const result = normalizeMilitaryInstallations(
    {
      elements: [
        {
          type: 'way',
          id: 7,
          center: { lat: 12, lon: 77 },
          tags: { military: 'airfield', name: 'Example' },
          geometry: [
            { lat: 12, lon: 77 },
            { lat: 12.1, lon: 77 },
            { lat: 12, lon: 77.1 },
...
test('an unnamed feature reads as its class, never as a raw OSM id', () => {
  ...
  for (const record of result.records) {
    assert.doesNotMatch(
      record.name,
      /\d/,
      `label must carry no OSM id: ${record.name}`,
    );
    assert.doesNotMatch(
      record.name,
      /[()]/,
      `label must carry no id parenthetical: ${record.name}`,
    );
  }
    ...
    elements: [
      {
        ...
        tags: { military: 'range', name: 'Camp Swift' },
      },
      {
        type: 'way',
        id: 2,
        center: { lat: 30.1, lon: -97.1 },
        tags: { military: 'airfield', 'name:en': 'Bergstrom' },
      },
    ],
  });
  assert.deepEqual(
    result.records.map((record) => record.name),
    ['Camp Swift', 'Bergstrom'],
  );
});
...
test('an unmapped class title-cases instead of leaking an underscored tag', () => {
  ...
  assert.equal(
    humanizeInstallationClass('training_area_north'),
    'Training area north',
  );
...
test('accepts only small non-dateline request bboxes', () => {
  ...
  assert.equal(
    isValidInstallationBoundingBox({ south: -20, west: 0, north: 20, east: 1 }),
    false,
  );
});
...
test('a way with only bounds is kept, at the midpoint of that box', () => {
      ...
      elements: [
        {
          type: 'way',
          id: 92701457,
          bounds: { minlat: 51.94, minlon: 7.98, maxlat: 51.96, maxlon: 8.02 },
  ...
  assert.equal(result.records[0].longitude, 8.0);
});
    ...
    elements: [
      {
        type: 'relation',
        id: 5,
        bounds: { minlat: -1, minlon: -2, maxlat: 1, maxlon: 2 },
        tags: { landuse: 'military' },
      },
    ],
  });
  ...
  const result = normalizeMilitaryInstallations({
    elements: [
      // latitude past the pole
      {
        type: 'way',
        id: 1,
        bounds: { minlat: 80, minlon: 0, maxlat: 95, maxlon: 1 },
        tags: { military: 'range' },
      },
      // longitude past the antimeridian
      {
        type: 'way',
        id: 2,
        bounds: { minlat: 0, minlon: 170, maxlat: 1, maxlon: 181 },
        tags: { military: 'range' },
      },
  ...
  const result = normalizeMilitaryInstallations({
    elements: [
      // south above north
      {
        type: 'way',
        id: 6,
        bounds: { minlat: 10, minlon: 0, maxlat: 8, maxlon: 1 },
        tags: { military: 'range' },
      },
      // west east of east: a dateline-spanning box would midpoint to longitude 0
      {
        type: 'way',
        id: 7,
        bounds: { minlat: 5, minlon: 179, maxlat: 6, maxlon: -179 },
        tags: { military: 'range' },
      },
