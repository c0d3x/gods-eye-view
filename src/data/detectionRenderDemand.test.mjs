...
test('detection holds nothing, and asks for its own frames instead', async () => {
  ...
  assert.match(
    source,
    /if\s*\(\s*detectionNeedsFollowUpFrame\(\s*\{\s*[\s\S]*?,?\s*\},?\s*\),?\s*\)\s*\{\s*governorRequestRender\(\s*'detection-animation',?\s*\);/,
  ...
  const emptyExit =
    /\/\/ Drop the replay buffer with it[\s\S]*?return \{\s*didSolve: false[^}]*\}/.exec(
      source,
    )?.[0];
  ...
  assert.match(
    shouldPaint,
    /if\s*\(\s*decision\s*\.requestFollowUp,?\s*\)\s*governorRequestRender\(\s*'detection-paint-skipped',?\s*\)/,
...
test('a layer whose detectable set changed dirties the solve', async () => {
  ...
  const manager = await readFile(
    new URL('./manager.js', import.meta.url),
    'utf8',
  );
  assert.match(
    manager,
  ...
  assert.match(
    manager,
    /governorRequestRender\(`layer-tick:\$\{layerId\}`\);[\s\S]{0,700}?markDetectionSourcesChanged\(`layer-tick:\$\{layerId\}`\);/,
    'a poll tick marks the solve dirty alongside its render request',
  );
  assert.match(
    manager,
    /governorRequestRender\('layer-visibility'\);[\s\S]{0,400}?markDetectionSourcesChanged\('layer-visibility'\);/,
...
test('the render-governor gate covers the parked case, with teeth on the painter', async () => {
  ...
  assert.match(
    gate,
    /else\s*\{\s*if\s*\(\s*consecutive\s*>\s*0,?\s*\)\s*restarts\s*\+=\s*1;\s*consecutive\s*=\s*0;,?\s*\}/,
    'any activity restarts the run, so it cannot straddle the deferred burst',
  );
  assert.match(
    gate,
    /check\(\s*'the\s*parked\s*scene\s*holds\s*a\s*settled\s*quiet\s*run\s*before\s*the\s*idle\s*window\s*is\s*counted',\s*idleSettle\s*\.quiet/,
    'quiet that never arrives is asserted as a failure, not skipped past',
  );
  assert.match(
    gate,
    /check\(\s*'the\s*scene\s*holds\s*a\s*settled\s*quiet\s*run\s*again\s*before\s*the\s*second\s*idle\s*window',\s*teardownSettle\s*\.quiet/,
  );
  assert.match(
    gate,
...
test('aircraft brackets stay prompt because the aircraft layers hold the render loop', async () => {
  ...
  for (const file of ['./aircraftLayerCore.js']) {
    const source = await readFile(new URL(file, import.meta.url), 'utf8');
    const enable = /\n( +)enable\([\s\S]*?\n\1\},/.exec(source)?.[0];
    assert.ok(enable, `${file}: enable() is still identifiable`);
    assert.match(
      enable,
      /holdContinuousRender\((config\.id|'(flights|military)')\)/,
    ...
    assert.match(
      source,
      /releaseContinuousRender\((config\.id|'(flights|military)')\)/,
