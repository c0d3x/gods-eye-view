...
async function launch(overrides = {}, dotenv = '', script = 'dev-cctv.sh') {
  ...
  try {
    ...
    await fs.mkdir(path.join(root, 'src', 'editions', 'local'), {
      recursive: true,
    });
    await fs.copyFile(
      new URL('./editions/local/data.js', import.meta.url),
      path.join(root, 'src', 'editions', 'local', 'data.js'),
    );
...
bashTest(
  'the launcher hands keys to the dev server without putting them in an argument list',
  async () => {
    const result = await launch({
      OPENAI_API_KEY: 'fixture-argv-probe-voice',
      TOMTOM_API_KEY: 'fixture-argv-probe-traffic',
    });
...
bashTest(
  'the secure launcher pins the loopback address, whatever HOST says, and starts keyless',
  async () => {
...
bashTest(
  'the secure launcher reads keys from .env like the normal launcher',
  async () => {
    const result = await launch(
      {},
