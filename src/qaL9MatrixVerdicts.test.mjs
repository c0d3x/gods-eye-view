...
test('a SKIPPED verdict must name its reason', () => {
  ...
  assert.equal(
    normalizeVerdict({ status: SKIP, detail: 'x', tag: 'NOPE' }).status,
    CRASH,
  );
  assert.equal(
    normalizeVerdict({ status: SKIP, detail: 'x', tag: 'ENV' }).status,
    SKIP,
  );
});
...
test('a missing scoreboard is a harness crash, not an environment skip', () => {
  ...
  const v = classifyNoScoreboard(
    'RESULT',
    run({ code: 2, err: 'TypeError: boom' }),
  );
  assert.equal(v.status, CRASH);
});
...
test('ENV is only claimed when the harness itself says the server was absent', () => {
  const v = classifyNoScoreboard(
    'RESULT',
    run({ code: 2, err: 'Dev server not reachable at http://localhost:4220' }),
  );
...
test('unrun assertions are surfaced, never folded into a plain pass', () => {
  const v = readResultLine(
    run({ out: 'RESULT: 9 passed, 0 failed, 3 skipped' }),
  );
...
test('a scoreboard that asserted nothing is not a pass', () => {
  assert.equal(
    readResultLine(run({ out: 'RESULT: 0 passed, 0 failed' })).status,
    CRASH,
  );
});
...
test('failures and post-report exits are never green', () => {
  assert.equal(
    readResultLine(run({ out: 'RESULT: 8 passed, 1 failed' })).status,
    FAIL,
  );
  assert.equal(
    readResultLine(run({ code: 3, out: 'RESULT: 8 passed, 0 failed' })).status,
    CRASH,
  );
  assert.equal(readResultLine(run({ timedOut: true })).status, FAIL);
  assert.equal(
    readResultLine(run({ out: 'RESULT: 8 passed, 0 failed' })).status,
    PASS,
  );
});
...
test('a scoreboard line with trailing garbage is not a scoreboard', () => {
  ...
  const v = readResultLine(
    run({ out: 'RESULT: 8 passed, 0 failed AND THEN EVERYTHING EXPLODED' }),
  );
  assert.equal(v.status, CRASH);
});
...
test('two contradictory scoreboards are a crash, not "first one wins"', () => {
  const v = readResultLine(
    run({ out: 'RESULT: 8 passed, 0 failed\nRESULT: 3 passed, 5 failed' }),
  );
  assert.equal(v.status, CRASH);
...
test('duplicate cockpit and floor verdicts are crashes too', () => {
  assert.equal(
    readCockpit(
      run({
        out: 'RESULT: READY (0 failures)\nRESULT: NOT_READY (2 failures)',
      }),
    ).status,
    CRASH,
  );
  assert.equal(
    readFloorVerdict(run({ out: 'VERDICT: PASS\nVERDICT: FAIL' })).status,
    CRASH,
  );
});
...
test('a single well-formed line still parses, indentation and all', () => {
  ...
  assert.equal(
    readCockpit(run({ out: '  RESULT: READY (0 failures)' })).status,
    PASS,
  );
  assert.equal(
    readFloorVerdict(run({ out: 'low contacts...\nVERDICT: PASS' })).status,
    PASS,
  );
});
...
test('the engines floor is enforced, not just the major version', () => {
  ...
  assert.equal(
    satisfiesEngines('24.0.0', range),
    false,
    '24.0.0 is below the 24.14.0 floor',
  );
...
test('only a parseable Node 24 counts as the calibrated allocation runtime', () => {
  ...
  assert.equal(
    isCalibratedAllocationRuntime('24.19.0'),
    true,
    'the v prefix is optional',
  );
  assert.equal(
    isCalibratedAllocationRuntime('v25.6.1'),
    false,
    'the npm re-resolution case',
  );
  assert.equal(isCalibratedAllocationRuntime('v22.21.1'), false);
  for (const junk of ['', '   ', 'node', undefined, null]) {
    assert.equal(
      isCalibratedAllocationRuntime(junk),
      false,
      `unparseable input must not pass: ${junk}`,
    );
  }
});
...
test('a traffic result that landed empty keeps failing', () => {
  ...
  assert.equal(
    trafficFlowInconclusive({ mode: 'live', tilesFetched: 0, loading: false }),
    false,
  ...
  assert.equal(
    trafficFlowInconclusive({
      ...
      tilesFetched: 0,
      loading: true,
      error: 'flow status unavailable',
    }),
    false,
    'an error is an answer — it must not be softened',
  );
  assert.equal(
    trafficFlowInconclusive({ mode: 'sim', tilesFetched: 0, loading: true }),
    false,
...
test('cockpit READY passes only at zero failures', () => {
  ...
  assert.equal(
    readCockpit(run({ out: 'RESULT: NOT_READY (2 failures)' })).status,
    FAIL,
  );
});
...
test('an inconclusive floor verdict fails the oracle, it does not skip it', () => {
  const v = readFloorVerdict(
    run({
      out: 'low contacts with plausible mesh readings: 0; buried (< -2m): 0\nVERDICT: INCONCLUSIVE',
    }),
  );
...
test('an unknown key state fails; it never becomes an owner-run skip', () => {
  ...
  assert.equal(
    keyGuard('FIRMS', true),
    null,
    'a known key state does not short-circuit',
  );
...
test('a known condition annotates a failure without changing it', () => {
  const conditions = [
    { when: /no console errors[\s\S]{0,300}?503/, note: 'not key-tolerant' },
  ];
  const annotated = applyKnownConditions(
...
test('a known condition is evidence-gated and never touches a pass', () => {
  ...
  assert.equal(
    applyKnownConditions(fail('x'), conditions, 'unrelated output').note,
    undefined,
  );
  assert.equal(
    applyKnownConditions(
...
test('expectation and exemption lists never overlap', () => {
  for (const id of Object.keys(CREDIT_EXEMPT_LAYERS)) {
    assert.equal(
      CREDIT_EXPECTATIONS[id],
      undefined,
      `${id} cannot be both expected and exempt`,
    );
  }
});
