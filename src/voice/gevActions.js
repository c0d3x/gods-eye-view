...
async function resolveRadioLocation(
  ...
  if (!apiKey)
    throw new Error(
      'No Google Maps API key available for Radio location search',
    );
    ...
    context:
      typeof styleManager.getContextModeState === 'function'
        ? {
            ...
            ...(activeContactsWindow()
              ? { contactsWindow: activeContactsWindow() }
              : {}),
          }
        : null,
    cockpit:
...
function aircraftProximityWindowForQuery(args, result) {
  ...
  const window = collectAircraftProximityWindow(subject.position, {
    radiusM,
    subject,
  });
  ...
  const request = (async () => {
    try {
      ...
      const labels = uniqueStrings(
        relevantResults.map((item) => item.formatted_address),
      ).slice(0, 12);
      const streetLabels = uniqueStrings(
        relevantResults.flatMap((item) => {
          const itemComponents = Array.isArray(item.address_components)
  ...
  const places = [samples[0], samples[1], samples[2]]
    ...
    .flatMap((sample) => {
      const place = reverseGeocodeCache.get(
        reverseGeocodeKey(sample.latitude, sample.longitude),
      );
      if (!place) return [];
      return [
        {
  ...
  const request = (async () => {
    try {
      ...
      const response = await fetchWithTimeout(
        `/api/google/nearby-places?${params}`,
        {},
        5000,
      );
      const data = await response.json().catch(() => null);
      const places =
        response.ok && Array.isArray(data?.places)
...
function activeContactsWindow() {
  try {
    return contactsWindowFromSnapshot(
