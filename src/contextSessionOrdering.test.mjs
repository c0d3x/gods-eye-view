...
test('Clear All defers a newer explicit mission guard until its batch settles', () => {
  const attachStart = src.indexOf('attachDataManager(dataManager) {');
  const attachEnd = src.indexOf(
    'if (typeof this._dataManager?.subscribeBeforeDestroy',
    attachStart,
  );
  const attach = src.slice(attachStart, attachEnd);
...
test('context handler: either failed direct Context-shell start rolls the session back', () => {
  ...
  assert.match(
    failedBranch,
    /\[\s*'military-awareness',\s*'rocket-launches',?\s*\]\s*\.includes\(\s*change\s*\.layerId,?\s*\)/,
  );
  assert.match(
    failedBranch,
...
test('only the expanded Radio Enable gesture requests the contained post-enable reveal', () => {
  ...
  assert.match(
    radioControls,
    /if\s*\(\s*revealAfterEnable,?\s*\)\s*await\s*this\s*\._revealRadioControlsAfterExplicitEnable\(\s*trigger,?\s*\)/,
  );
  assert.equal(
    (src.match(/_revealRadioControlsAfterExplicitEnable\(trigger\)/g) || [])
      .length,
    2,
  );
});
...
test('right-rail context entry is transactional: activation result gates the mode', () => {
  ...
  assert.match(
    failureBlock,
    /activationError\s*\|\|\s*activated\s*===\s*false\s*\|\|\s*!this\s*\._dataManager\s*\.isEnabled\(\s*entryLayerId,?\s*\)/,
  );
  assert.match(
    failureBlock,
    /cancelledAndSettled\s*=\s*terminalIntentOutcome\s*\?\s*\.succeeded\s*===\s*false/,
  );
  assert.match(
    failureBlock,
...
test('Context entry awaits isolation and direct shell routes isolate in the visibility guard', () => {
  ...
  assert.match(
    select,
    /await\s*this\s*\._clearLayersOutsideContextMode\(\s*mode,\s*\{\s*notificationToken,\s*signal,?\s*\},?\s*\)/,
  );
  assert.match(
    select,
  ...
  const guard = src.slice(
    looseIndexOf(
      src,
      'this._dataManagerVisibilityGuardUnsubscribe = this._dataManager.addVisibilityGuard',
    ),
    looseIndexOf(
      src,
  ...
  assert.match(
    guard,
    /const\s*notificationToken\s*=\s*change\s*\.notificationToken\s*\|\|\s*Symbol\(\s*'direct-context-shell-entry',?\s*\)/,
  );
  assert.match(
    guard,
  ...
  assert.match(
    guard,
    /await\s*this\s*\._clearLayersOutsideContextMode\(\s*entryMode,\s*\{\s*notificationToken,?\s*\},?\s*\)/,
  );
  assert.match(
    guard,
  ...
  assert.match(
    guard,
    /this\s*\._userFacingContextNotificationTokens\s*\.delete\(\s*notificationToken,?\s*\)/,
  );
});
...
test('Context cancellation reaches isolation and restore lifecycle mutations', () => {
  ...
  const clear = src.slice(
    clearStart,
    src.indexOf('_handleContextLayerChange(', clearStart),
  );
...
test('Context production rollback paths merge primary and restore failed-layer identities', () => {
  ...
  assert.match(
    select,
    /transitionError\s*\.failedLayerIds\s*=\s*\[\s*\.\.\s*\.new\s*Set\(\s*\[\s*/,
  );
  assert.match(
    select,
    /this\s*\._contextTransitionFailedLayerIds\s*=\s*\[\s*\.\.\.\(\s*transitionError\s*\.failedLayerIds\s*\|\|\s*\[,?\s*\],?\s*\),?\s*\]/,
  );
});
