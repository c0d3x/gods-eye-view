/**
 * TomTom traffic-flow proxy: /api/tomtom/status and
 * /api/tomtom/flow/{z}/{x}/{y}.pbf, with memory and disk tile caches and a
 ...
 * @param {string} [options.cacheDir] Where tiles and budget.json are kept;
 *   .gev-cache/tomtom by default.
 * @returns {import('vite').Plugin}
 */
export function tomtomProxy({
  cacheDir = DISK_CACHE_LIMITS.tomtom.directory,
} = {}) {
  const TILE_TTL_MS = 120_000;
  const CACHE_DIR = cacheDir;
  // Another directory, such as a test's, is pruned on its own.
  const pruner =
    cacheDir === DISK_CACHE_LIMITS.tomtom.directory
      ? diskCachePruners.tomtom
      : createCachePruner({ ...DISK_CACHE_LIMITS.tomtom, directory: cacheDir });
