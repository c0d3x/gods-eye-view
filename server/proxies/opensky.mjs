/**
 * OpenSky Network proxy: /api/opensky serves worldwide aircraft state vectors
 * with an OAuth client-credentials token and a credit-aware response cache,
 * and falls back to a 250 nm adsb.lol snapshot around the view when OpenSky
 * can't answer.
 */
...
// Deadlines for the token and state-vector calls, and the cap on a
      ...
      server.middlewares.use('/api/opensky', async (req, res) => {
        try {
          ...
          if (upstream.status === 429) {
            ...
            const retryAfterSec = numericHeader(
              upstream.headers,
          ...
          if (upstream.status === 401 || upstream.status === 403) {
            ...
            console.warn(
              `[OpenSky Proxy] ${describeUpstreamFailure(upstream.status, body)}`,
            );
            body = JSON.stringify({
              error: upstreamErrorMessage('OpenSky', upstream.status),
            });
          }
          ...
          res.end(
            JSON.stringify({
              error:
                status === 504
                  ? 'OpenSky did not answer in time'
                  : 'OpenSky proxy error',
            }),
          );
        }
